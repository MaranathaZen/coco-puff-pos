...
export default function Layout() {
  ...
  const menus = [
    ...
    { to: '/pengaturan', icon: Settings,         label: 'Pengaturan', roles: ['owner', 'manager'] },
  ].filter(m => user && m.roles.includes(user.role))

  // Nama toko dari store object, fallback ke APP_NAME
          ...
          {menus.map(({ to, icon: Icon, label }) => (
            <NavLink key={to} to={to}
              className={({ isActive }) => cn(
                'flex-1 flex flex-col items-center py-2.5 gap-1 text-xs transition-colors touch-manipulation',
