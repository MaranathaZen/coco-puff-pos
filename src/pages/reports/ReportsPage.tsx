...
// Owner/Manager: lihat semua toko semua divisi
// Gudang: lihat laporan gudang saja
...
import { RefreshCw, TrendingUp, ShoppingBag, Package, Receipt, ArrowRightLeft, FlaskConical } from 'lucide-react'
...
export default function ReportsPage() {
  ...
  const purchases = useLiveQuery(async () =>
    db.purchases.filter(p =>
      p.created_at >= dateRange.start &&
      p.created_at <= dateRange.end
    ).toArray()
  , [dateRange])

  const expenses = useLiveQuery(async () =>
    db.warehouse_expenses.filter(e =>
  ...
  const productionLogs = useLiveQuery(async () =>
    db.production_logs.filter(l =>
      l.created_at >= dateRange.start &&
      l.created_at <= dateRange.end
    ).toArray()
  , [dateRange])
  ...
  const byMethod = useMemo(() => {
    ...
    for (const t of transactions || []) {
      map[t.payment_method] = (map[t.payment_method] || 0) + t.total
    }
    return map
  }, [transactions])
  ...
  // ── Per bahan (top pembelian) ─────────────────────────────
  const topMaterials = useMemo(async () => [], []) // placeholder

  const periodLabel = { hari: 'Hari Ini', minggu: '7 Hari', bulan: 'Bulan Ini', custom: 'Custom' }
          ...
          <>
            ...
            <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
              ...
              {Object.keys(byMethod).length === 0 && (
                <div className="py-8 text-center text-sm text-gray-400">Belum ada data transaksi</div>
              )}
            </div>
            ...
            <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
              ...
              {(materials || [])
                .map(m => {
                  ...
                  return { ...m, qty, nilai: qty * (m.unit_cost || 0) }
                })
                ...
                .map((m, idx) => (
                  <div key={m.id} className={`flex items-center justify-between px-4 py-2.5 ${idx !== 0 ? 'border-t border-gray-50' : ''} ${m.qty <= m.min_stock && m.min_stock > 0 ? 'bg-red-50/30' : ''}`}>
                    <div>
                      ...
                      <p className="text-xs text-gray-400">Avg {formatRupiah(m.unit_cost || 0)}/{m.unit}</p>
                    </div>
                    <div className="text-right">
                      <p className={`text-sm font-semibold ${m.qty <= m.min_stock && m.min_stock > 0 ? 'text-red-500' : 'text-gray-900'}`}>
                        {m.qty} {m.unit}
                      </p>
              ...
              {(materials || []).length === 0 && (
                <div className="py-8 text-center text-sm text-gray-400">Belum ada data bahan</div>
              )}
            </div>
          </>
        )}
          ...
          <>
            ...
            <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
              ...
              {byExpenseCat.length === 0 && (
                <div className="py-8 text-center text-sm text-gray-400">Belum ada data biaya</div>
              )}
            </div>
                ...
                <div key={e.id} className={`flex items-center justify-between px-4 py-2.5 ${idx !== 0 ? 'border-t border-gray-50' : ''}`}>
                  <div>
                    ...
                    <p className="text-xs text-gray-400">
                      {katLabel[e.category] || e.category} · {new Date(e.created_at).toLocaleDateString('id-ID', { day:'numeric', month:'short' })}
                    </p>
                  </div>
...
function ProductionStokSection() {
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
      ...
      {(fgStocks || []).length === 0 && (
        <div className="py-8 text-center text-sm text-gray-400">Belum ada produk jadi</div>
      )}
    </div>
  )
}
