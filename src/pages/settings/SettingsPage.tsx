  ...
  async function handleSave() {
    ...
    try {
      if (isNew) {
        ...
        await addToSyncQueue('stores', newStore.id, 'upsert' as any, newStore, newStore.id)
        ...
        await addToSyncQueue('stores', store.id, 'upsert' as any, updated, store.id)
...
function ChangePasswordTab({ userId, storeId }: { userId: string; storeId: string }) {
  ...
  async function handleChange() {
    ...
    if (newPass.length<6) return toast.error('Password/PIN minimal 6 karakter')
    setSaving(true)
    try {
      const user = await db.users.get(userId); if (!user) return
      const { ok } = await verifyPassword(oldPass, user.password_hash)
      if (!ok) { toast.error('Password lama salah'); return }
    ...
    <div className="p-4">
      <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
        ...
        <div className="px-4 py-3">
          ...
          <input className="w-full text-sm text-gray-900 outline-none bg-transparent" type="password" value={newPass} onChange={e=>setNew(e.target.value)} placeholder="Min. 6 karakter" />
        </div>
      </div>
...
function PromoTab({ currentUser }: { currentUser: User }) {
  ...
  async function toggleActive(p: PromoItem){const upd={...p,is_active:!p.is_active};setPromos(prev=>prev.map(x=>x.id===p.id?upd:x));await db.promotions.put(upd as any);await addToSyncQueue('promotions',upd.id,'upsert' as any,upd,(upd as any).store_id||'gudang')}
...
function ResetDataTab() {
  const { user } = useAuthStore()
  const region = () => getActiveRegion(user)   // reset HANYA region user aktif (owner02->bali, owner01->malang)
  // Hapus baris child (tak punya kolom region) berdasar ID induk di region ini.
  async function delChildren(childTable: string, parentTable: string, fk: string) {
    const { data } = await supabase.from(parentTable).select('id').eq('region', region())
    const ids = (data || []).map((r: any) => r.id)
    for (let i = 0; i < ids.length; i += 200) { const b = ids.slice(i, i + 200); if (b.length) await supabase.from(childTable).delete().in(fk, b) }
  }
  const delParent = (t: string) => supabase.from(t).delete().eq('region', region())
  ...
  async function resetToko(){if(!confirm(`Reset data TOKO region ${region().toUpperCase()}? Transaksi, shift, close order region ini akan dihapus. Region lain AMAN.`))return;setResetting(true);setLog([]);try{addLog('🗑️ Transaksi region '+region()+'...');await delChildren('transaction_items','transactions','transaction_id');await delParent('transactions');await delParent('shifts');addLog('🗑️ Close order...');await delParent('close_order_reports');addLog('🧹 Membersihkan lokal...');await db.transaction_items.clear();await db.transactions.clear();await db.shifts.clear();await db.stock.clear();await db.sync_queue.clear();if((db as any).close_order_reports)await(db as any).close_order_reports.clear();clearLocalStorageSaldo();addLog('✅ Selesai');toast.success('Data toko '+region()+' direset')}catch(e){toast.error('Gagal: '+String(e));addLog('❌ '+String(e))}finally{setResetting(false)}}
  async function resetGudang(){if(!confirm(`Reset data GUDANG region ${region().toUpperCase()}? Region lain AMAN.`))return;setResetting(true);setLog([]);try{addLog('Menghapus server region '+region()+'...');await delChildren('warehouse_mutation_items','warehouse_mutations','mutation_id');await delParent('warehouse_mutations');await delChildren('purchase_items','purchases','purchase_id');await delParent('purchases');await delParent('warehouse_expenses');addLog('Membersihkan lokal...');await db.warehouse_mutation_items.clear();await db.warehouse_mutations.clear();await db.purchase_items.clear();await db.purchases.clear();await db.warehouse_expenses.clear();await db.warehouse_stock.clear();addLog('✅ Selesai');toast.success('Data gudang '+region()+' direset')}catch(e){toast.error('Gagal: '+String(e));addLog('❌ '+String(e))}finally{setResetting(false)}}
  async function resetProduksi(){if(!confirm(`Reset data PRODUKSI region ${region().toUpperCase()}? Region lain AMAN.`))return;setResetting(true);setLog([]);try{addLog('Menghapus server region '+region()+'...');await delChildren('production_log_materials','production_logs','log_id');await delParent('production_logs');await delChildren('production_mutation_items','production_mutations','mutation_id');await delParent('production_mutations');addLog('Membersihkan lokal...');await db.production_log_materials.clear();await db.production_logs.clear();await db.production_mutation_items.clear();await db.production_mutations.clear();await db.production_stock.clear();await db.finished_goods_stock.clear();addLog('✅ Selesai');toast.success('Data produksi '+region()+' direset')}catch(e){toast.error('Gagal: '+String(e));addLog('❌ '+String(e))}finally{setResetting(false)}}
  async function resetSettings(){if(!confirm(`Reset promo region ${region().toUpperCase()}?`))return;setResetting(true);setLog([]);try{addLog('Mereset promo...');await delParent('promotions');await db.promotions.clear();addLog('✅ Selesai');toast.success('Promo '+region()+' direset')}catch(e){toast.error('Gagal: '+String(e));addLog('❌ '+String(e))}finally{setResetting(false)}}
  async function resetSemua(){const ok1=confirm(`PERHATIAN!\n\nHapus SEMUA data operasional region ${region().toUpperCase()} SAJA.\nRegion lain & master data tetap AMAN.\n\nLanjutkan?`);if(!ok1)return;const typed=prompt('Ketik RESET untuk konfirmasi:');if(typed!=='RESET'){toast.error('Reset dibatalkan');return}setResetting(true);setLog([]);try{addLog('🗑️ Menghapus data region '+region()+'...');await delChildren('transaction_items','transactions','transaction_id');await delParent('transactions');await delParent('shifts');await delParent('close_order_reports');await delChildren('warehouse_mutation_items','warehouse_mutations','mutation_id');await delParent('warehouse_mutations');await delChildren('purchase_items','purchases','purchase_id');await delParent('purchases');await delParent('warehouse_expenses');await delChildren('production_log_materials','production_logs','log_id');await delParent('production_logs');await delChildren('production_mutation_items','production_mutations','mutation_id');await delParent('production_mutations');await delParent('promotions');addLog('🧹 Membersihkan lokal...');await db.transaction_items.clear();await db.transactions.clear();await db.shifts.clear();await db.warehouse_mutation_items.clear();await db.warehouse_mutations.clear();await db.purchase_items.clear();await db.purchases.clear();await db.warehouse_expenses.clear();await db.production_log_materials.clear();await db.production_logs.clear();await db.production_mutation_items.clear();await db.production_mutations.clear();await db.production_stock.clear();await db.finished_goods_stock.clear();await db.warehouse_stock.clear();await db.stock.clear();await db.promotions.clear();await db.sync_queue.clear();if((db as any).close_order_reports)await(db as any).close_order_reports.clear();clearLocalStorageSaldo();addLog('✅ Selesai — region '+region()+' siap Go-Live!');toast.success('Data region '+region()+' direset. Siap Go-Live!')}catch(e){toast.error('Gagal: '+String(e));addLog('❌ Error: '+String((e as any)?.message||e))}finally{setResetting(false)}}
  return (
    <div className="p-4 space-y-4">
